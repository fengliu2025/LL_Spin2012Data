...
const double L_mass_width_estimate = 1.4e-3; //estimated peak width
...
const float Delta_y_cut = 0.5;
const float Delta_phi_cut = TMath::Pi()/3.;
...
//calculate theta star for Lmabdda(-bar) pair
pair<double,double> LpairThetaStar_prodPlane(TLorentzVector *L1, TLorentzVector *p1, TLorentzVector *L2, TLorentzVector *p2)
{
  //normal vector to production plane defined by momenta of the Lambda pair
  TVector3 prod_plane_vect = L1->Vect().Cross(L2->Vect());

  p1->Boost(-L1->BoostVector());
  double theta_star1 = p1->Angle(prod_plane_vect);

  p2->Boost(-L2->BoostVector());
  double theta_star2 = p2->Angle(prod_plane_vect);

  pair<double,double> ThetaStar_prodPlane_pair(theta_star1, theta_star2);

  return ThetaStar_prodPlane_pair;
}
...
//calculate theta star for Lmabdda(-bar) pair
double LpairThetaStar_CMS(TLorentzVector *L1, TLorentzVector *p1, TLorentzVector *L2, TLorentzVector *p2)
{
  //first boos to CMS of the pair
  TLorentzVector L_pair_4mom = *L1 + *L2;

  L1->Boost(-L_pair_4mom.BoostVector());
  L2->Boost(-L_pair_4mom.BoostVector());

  p1->Boost(-L_pair_4mom.BoostVector());
  p2->Boost(-L_pair_4mom.BoostVector());

  //-------------------------------

  //then boost protons to mother rest frame
  p1->Boost(-L1->BoostVector());
  p2->Boost(-L2->BoostVector());
...
//calculate Minv of L pair in CMS of the pair
double LpairMinv_CMS(TLorentzVector *L1, TLorentzVector *L2)
{
  //first boos to CMS of the pair
  TLorentzVector L_pair_4mom = *L1 + *L2;
  ...
  TLorentzVector L_pair_4mom_CMS = *L1 + *L2;

  return L_pair_4mom_CMS.M();
}
...
//calculate momentun of L in CMS of the pair
//momentum of first is returned (should not matter im CMS)
double L_mom_CMS(TLorentzVector *L1, TLorentzVector *L2)
{
  //first boos to CMS of the pair
  TLorentzVector L_pair_4mom = *L1 + *L2;
...
bool cuts(int year, float L_y, int hasTOFinfo, float p_DCA, float pi_DCA, float pair_DCA, float dec_L, float theta)
{
  ...
  if( cos(theta) < 0.996 ) return false;

  if( (sin(theta)*dec_L) > 1.0 ) return false; //alternative analysis cut
...
bool cuts_topo_sys_err(float pair_DCA, float dec_L, float theta)
{
  if( pair_DCA > 0.9 ) return false;

  if( dec_L < 3 ) return false;

  if( cos(theta) < 0.997 ) return false;

  return true;
}

//for tight topological cuts for sys. err.
bool cuts_DCA_daughters_sys_err(float p_DCA, float pi_DCA)
{
...
bool DoAnalysis(TChain *L_tree, const int ReadMode, const int cut_type = 0, const int energy = 510, const int year = 2017, const int test = 0)
{
  ...
  //some topology distributions

  //DCA to PV of L and Lbar
  TH1F *L_DCA_PV_US = new TH1F("L_DCA_PV_US", "L_DCA_PV_US", 100, -5, 5);
  TH1F *L_DCA_PV_LS = new TH1F("L_DCA_PV_LS", "L_DCA_PV_LS", 100, -5, 5);

  TH1F *Lbar_DCA_PV_US = new TH1F("Lbar_DCA_PV_US", "Lbar_DCA_PV_US", 100, -5, 5);
  TH1F *Lbar_DCA_PV_LS = new TH1F("Lbar_DCA_PV_LS", "Lbar_DCA_PV_LS", 100, -5, 5);

  //cos of pointing algle
  TH1F *L_cos_theta_US = new TH1F("L_cos_theta_US", "L_cos_theta_US", 100, 0.995, 1.);
  TH1F *Lbar_cos_theta_US = new TH1F("Lbar_cos_theta_US", "Lbar_cos_theta_US", 100, 0.995, 1.);

  TH1F *L_cos_theta_LS = new TH1F("L_cos_theta_LS", "L_cos_theta_LS", 100, 0.995, 1.);
  TH1F *Lbar_cos_theta_LS = new TH1F("Lbar_cos_theta_LS", "Lbar_cos_theta_LS", 100, 0.995, 1.);

  //decay length
  TH1F *L_DecL_US = new TH1F("L_DecL_US", "L_DecL_US", 100, 0., 20.);
  TH1F *Lbar_DecL_US = new TH1F("Lbar_DecL_US", "Lbar_DecL_US", 100, 0., 20.);

  TH1F *L_DecL_LS = new TH1F("L_DecL_LS", "L_DecL_LS", 100, 0., 20.);
  TH1F *Lbar_DecL_LS = new TH1F("Lbar_DecL_LS", "Lbar_DecL_LS", 100, 0., 20.);
  ...
  vector<float> Lbar_DCA_PV_vector;
  ...
  vector<float> L_Lbar_L_DCA_PV;
  vector<float> L_Lbar_Lbar_DCA_PV;


  vector<float> L_Lbar_cos_theta_prodPlane_1;
  vector<float> L_Lbar_cos_theta_prodPlane_2;
  ...
  vector<float> L_L_L1_DCA_PV;
  vector<float> L_L_L2_DCA_PV;
  ...
  vector<float> Lbar_Lbar_Lbar1_DCA_PV;
  vector<float> Lbar_Lbar_Lbar2_DCA_PV;
  ...
  vector<float> L_DCA_PV_vector_background;
  ...
  vector<float> Lbar_DCA_PV_vector_background;
  ...
  vector<float> L_Lbar_L_DCA_PV_back;
  vector<float> L_Lbar_Lbar_DCA_PV_back;


  vector<float> L_Lbar_cos_theta_prodPlane_1_back;
  vector<float> L_Lbar_cos_theta_prodPlane_2_back;
  ...
  vector<float> L_L_L1_DCA_PV_back;
  vector<float> L_L_L2_DCA_PV_back;
  ...
  vector<float> Lbar_Lbar_Lbar1_DCA_PV_back;
  vector<float> Lbar_Lbar_Lbar2_DCA_PV_back;
  ...
  vector<float> L_Lbar_L_DCA_PV_ME_SE;
  ...
  vector<float> L_Lbar_Lbar_DCA_PV_ME_SE;
  ...
  vector<float> L_Lbar_L_DCA_PV_ME_SE_back_US_LS;
  ...
  vector<float> L_Lbar_Lbar_DCA_PV_ME_SE_back_US_LS;
  ...
  vector<float> L_Lbar_L_DCA_PV_ME_SE_back_LS_US;
  ...
  vector<float> L_Lbar_Lbar_DCA_PV_ME_SE_back_LS_US;
  ...
  vector<float> L_L_L1_DCA_PV_ME_SE;
  ...
  vector<float> L_L_L2_DCA_PV_ME_SE;
  ...
  vector<float> L_L_L1_DCA_PV_ME_SE_back;
  ...
  vector<float> L_L_L2_DCA_PV_ME_SE_back;
  ...
  vector<float> Lbar_Lbar_Lbar1_DCA_PV_ME_SE;
  ...
  vector<float> Lbar_Lbar_Lbar2_DCA_PV_ME_SE;
  ...
  vector<float> Lbar_Lbar_Lbar1_DCA_PV_ME_SE_back;
  ...
  vector<float> Lbar_Lbar_Lbar2_DCA_PV_ME_SE_back;
  ...
  vector<float> L_DCA_PV_vector_ME;
  ...
  vector<float> Lbar_DCA_PV_vector_ME;
  ...
  vector<float> L_DCA_PV_vector_ME_LS;
  ...
  vector<float> Lbar_DCA_PV_vector_ME_LS;
  ...
  for(Long64_t i = 0; i < nEntries; i++) //Read TTree only in ReadMode = 0
  {
    ...
    if(charge == 0)//US
    {
      if(p_ch == 1)//L
      {
        if( L_mass > L_mass_PDG-2*L_mass_width_estimate && L_mass < L_mass_PDG+2*L_mass_width_estimate)
        {
          ...
          L_DCA_PV_US->Fill(sin(L_theta)*L_decayL);
          L_cos_theta_US->Fill(cos(L_theta));
          L_DecL_US->Fill(L_decayL);
      ...
      if(p_ch == -1)//Lbar
      {
        if( L_mass > L_mass_PDG-2*L_mass_width_estimate && L_mass < L_mass_PDG+2*L_mass_width_estimate)
        {
          ...
          Lbar_DCA_PV_US->Fill(sin(L_theta)*L_decayL);
          Lbar_cos_theta_US->Fill(cos(L_theta));
          Lbar_DecL_US->Fill(L_decayL);
    ...
    if(charge != 0)//LS
    {
      if(p_ch == 1)//L
      {
        if( L_mass > L_mass_PDG-2*L_mass_width_estimate && L_mass < L_mass_PDG+2*L_mass_width_estimate)
        {
          ...
          L_DCA_PV_LS->Fill(sin(L_theta)*L_decayL);
          L_cos_theta_LS->Fill(cos(L_theta));
          L_DecL_LS->Fill(L_decayL);
      ...
      if(p_ch == -1)//Lbar
      {
        if( L_mass > L_mass_PDG-2*L_mass_width_estimate && L_mass < L_mass_PDG+2*L_mass_width_estimate)
        {
          ...
          Lbar_DCA_PV_LS->Fill(sin(L_theta)*L_decayL);
          Lbar_cos_theta_LS->Fill(cos(L_theta));
          Lbar_DecL_LS->Fill(L_decayL);
    ...
    if(eventId == eventID_last)
    {
      ...
      if(charge == 0)
      {
        ...
        if( cuts(year, L_y, pi_hasTOFinfo, p_dca, pi_dca, L_DCAdaughters, L_decayL, L_theta) && pT_bin_corr != -1 && eta_bin != -1 &&
            ( cut_type != 1 || cuts_topo_sys_err( L_DCAdaughters, L_decayL, L_theta ) ) && //cuts_topo_sys_err is evaluated only when cut_type != 1
            ( cut_type != 2 || cuts_pT_sys_err(p_pt, pi_pt) ) &&
            ( cut_type != 3 || cuts_DCA_daughters_sys_err(p_dca, pi_dca) ) )                                               //cuts_pT_sys_err is evaluated only when cut_type != 2
        {
          if( p_ch == 1 )
          {
            //QA histograms
            ...
            L_DCA_PV_vector.push_back(sin(L_theta)*L_decayL);
          ...
          else if( p_ch == -1 )
          {
            ...
            Lbar_DCA_PV_vector.push_back(sin(L_theta)*L_decayL);
      ...
      if(charge != 0) //same event as in previous iteration
      {
        //cuts
        if( cuts(year, L_y, pi_hasTOFinfo, p_dca, pi_dca, L_DCAdaughters, L_decayL, L_theta) && pT_bin_corr != -1 && eta_bin != -1 &&
            ( cut_type != 1 || cuts_topo_sys_err( L_DCAdaughters, L_decayL, L_theta ) ) &&
            ( cut_type != 2 || cuts_pT_sys_err(p_pt, pi_pt) ) &&
            ( cut_type != 3 || cuts_DCA_daughters_sys_err(p_dca, pi_dca) ) )
        //if( cuts(year, L_y, p_hasTOFinfo) && pT_bin_corr != -1 && eta_bin != -1)
        {
          if( p_ch == 1 )
          {
            ...
            L_DCA_PV_vector_background.push_back(sin(L_theta)*L_decayL);
          ...
          else if( p_ch == -1 )
          {
            ...
            Lbar_DCA_PV_vector_background.push_back(sin(L_theta)*L_decayL);
    ...
    else if(eventId != eventID_last)//new event
    {
      ...
      if(L_vector.size() > 0 && Lbar_vector.size() > 0)
      {
        ...
        for(unsigned int iLambda = 0; iLambda < L_vector.size(); iLambda++)
        {
          for(unsigned int iLambdaBar = 0; iLambdaBar < Lbar_vector.size(); iLambdaBar++)
          {
            ...
            if( L_vector.at(iLambda).M() > 1.105 && L_vector.at(iLambda).M() < 1.13 && Lbar_vector.at(iLambdaBar).M() > 1.105 && Lbar_vector.at(iLambdaBar).M() < 1.13 )
            {
              ...
              L_Lbar_L_DCA_PV.push_back(L_DCA_PV_vector.at(iLambda));
              L_Lbar_Lbar_DCA_PV.push_back(Lbar_DCA_PV_vector.at(iLambdaBar));

              //-------------------------------------------------------------------------

              pair<double,double> L_Lbar_pairThetaStar_prodPlane = LpairThetaStar_prodPlane(L_vector.at(iLambda), p_vector.at(iLambda), Lbar_vector.at(iLambdaBar), pBar_vector.at(iLambdaBar));

              L_Lbar_cos_theta_prodPlane_1.push_back(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.first));
              L_Lbar_cos_theta_prodPlane_2.push_back(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.second));
            }
      ...
      if(L_vector.size() > 1)
      {
        ...
        for(unsigned int iLambda1 = 0; iLambda1 < L_vector.size(); iLambda1++)
        {
          for(unsigned int iLambda2 = iLambda1+1; iLambda2 < L_vector.size(); iLambda2++)
          {
            ...
            L_L_L1_DCA_PV.push_back(L_DCA_PV_vector.at(iLambda1));
            L_L_L2_DCA_PV.push_back(L_DCA_PV_vector.at(iLambda2));
      ...
      if(Lbar_vector.size() > 1)
      {
        ...
        for(unsigned int iLambdaBar1 = 0; iLambdaBar1 < Lbar_vector.size(); iLambdaBar1++)
        {
          ...
          for(unsigned int iLambdaBar2 = iLambdaBar1+1; iLambdaBar2 < Lbar_vector.size(); iLambdaBar2++)
          {
            ...
            Lbar_Lbar_Lbar1_DCA_PV.push_back(Lbar_DCA_PV_vector.at(iLambdaBar1));
            Lbar_Lbar_Lbar2_DCA_PV.push_back(Lbar_DCA_PV_vector.at(iLambdaBar2));
      ...
      if(L_vector.size() > 0 && Lbar_vector_background.size() > 0)
      {
        ...
        for(unsigned int iLambda = 0; iLambda < L_vector.size(); iLambda++)
        {
          for(unsigned int iLambdaBar = 0; iLambdaBar < Lbar_vector_background.size(); iLambdaBar++)
          {
            ...
            if( L_vector.at(iLambda).M() > 1.105 && L_vector.at(iLambda).M() < 1.13 && Lbar_vector_background.at(iLambdaBar).M() > 1.105 && Lbar_vector_background.at(iLambdaBar).M() < 1.13 )
            {
              ...
              L_Lbar_L_DCA_PV_back.push_back(L_DCA_PV_vector.at(iLambda));
              L_Lbar_Lbar_DCA_PV_back.push_back(Lbar_DCA_PV_vector_background.at(iLambdaBar));
              ...
              pair<double,double> L_Lbar_pairThetaStar_prodPlane = LpairThetaStar_prodPlane(L_vector.at(iLambda), p_vector.at(iLambda), Lbar_vector_background.at(iLambdaBar), pBar_vector_background.at(iLambdaBar));

              L_Lbar_cos_theta_prodPlane_1_back.push_back(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.first));
              L_Lbar_cos_theta_prodPlane_2_back.push_back(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.second));
            }
      ...
      if(L_vector_background.size() > 0 && Lbar_vector.size() > 0)
      {
        for(unsigned int iLambda = 0; iLambda < L_vector_background.size(); iLambda++)
        {
          for(unsigned int iLambdaBar = 0; iLambdaBar < Lbar_vector.size(); iLambdaBar++)
          {
            ...
            if( L_vector_background.at(iLambda).M() > 1.105 && L_vector_background.at(iLambda).M() < 1.13 && Lbar_vector.at(iLambdaBar).M() > 1.105 && Lbar_vector.at(iLambdaBar).M() < 1.13 )
            {
              //save info for cos(theta*) to be analyzed later
              ...
              L_Lbar_L_DCA_PV_back.push_back(L_DCA_PV_vector_background.at(iLambda));
              L_Lbar_Lbar_DCA_PV_back.push_back(Lbar_DCA_PV_vector.at(iLambdaBar));
              ...
              pair<double,double> L_Lbar_pairThetaStar_prodPlane = LpairThetaStar_prodPlane(L_vector_background.at(iLambda), p_vector_background.at(iLambda), Lbar_vector.at(iLambdaBar), pBar_vector.at(iLambdaBar));
        ...
        for(unsigned int iLambda = 0; iLambda < L_vector.size(); iLambda++)
        {
          for(unsigned int iLambdaBckg = 0; iLambdaBckg < L_vector_background.size(); iLambdaBckg++)
          {
            ...
            if( nFillLL_bckg % 2 == 0 ) //fill LL pair wit US at x axis and LS at y axis
            {
              ...
              L_L_L1_DCA_PV_back.push_back(L_DCA_PV_vector.at(iLambda));
              L_L_L2_DCA_PV_back.push_back(L_DCA_PV_vector_background.at(iLambdaBckg));
            ...
            else //fill LL pair wit US at y axis and LS at x axis, i.e. opposite order than above
            {
              ...
              L_L_L1_DCA_PV_back.push_back(L_DCA_PV_vector_background.at(iLambdaBckg));
              L_L_L2_DCA_PV_back.push_back(L_DCA_PV_vector.at(iLambda));
        ...
        for(unsigned int iLambdaBar = 0; iLambdaBar < Lbar_vector.size(); iLambdaBar++)
        {
          for(unsigned int iLambdaBarBckg = 0; iLambdaBarBckg < Lbar_vector_background.size(); iLambdaBarBckg++)
          {
            ...
            if( nFillLbarLbar_backg % 2 == 0 ) //fill LL pair wit US at x axis and LS at y axis
            {
              ...
              Lbar_Lbar_Lbar1_DCA_PV_back.push_back(Lbar_DCA_PV_vector.at(iLambdaBar));
              Lbar_Lbar_Lbar2_DCA_PV_back.push_back(Lbar_DCA_PV_vector_background.at(iLambdaBarBckg));
            ...
            else //fill LL pair wit US at y axis and LS at x axis, i.e. opposite order than above
            {
              ...
              Lbar_Lbar_Lbar1_DCA_PV_back.push_back(Lbar_DCA_PV_vector_background.at(iLambdaBarBckg));
              Lbar_Lbar_Lbar2_DCA_PV_back.push_back(Lbar_DCA_PV_vector.at(iLambdaBar));
      ...
      if( L_vector.size() > 0 && Lbar_vector.size() > 0 && L_Lbar_L_vector_ME_SE.size() < 3e4 )
      {
        ...
        if( p_tag_vector.at(0) != piBar_tag_vector.at(0) && pBar_tag_vector.at(0) != pi_tag_vector.at(0) )
        {
          ...
          if( L_vector.at(0).M() > 1.105 && L_vector.at(0).M() < 1.13 && Lbar_vector.at(0).M() > 1.105 && Lbar_vector.at(0).M() < 1.13 )
          {
            ...
            L_Lbar_L_DCA_PV_ME_SE.push_back(L_DCA_PV_vector.at(0));
            ...
            L_Lbar_Lbar_DCA_PV_ME_SE.push_back(Lbar_DCA_PV_vector.at(0));
          }
        }
      }
      ...
      if( L_vector.size() > 1 && L_L_L1_vector_ME_SE.size() < 3e4)
      {
        ...
        {
          //Minv pre-selection to boost usable ME statistics
          if( L_vector.at(0).M() > 1.105 && L_vector.at(0).M() < 1.13 && L_vector.at(1).M() > 1.105 && L_vector.at(1).M() < 1.13  /*&& L_DCA_PV_vector.at(0) < L_DCA_cut  && L_DCA_PV_vector.at(1) < L_DCA_cut*/)
          {
            ...
            L_L_L1_DCA_PV_ME_SE.push_back(L_DCA_PV_vector.at(0));
            ...
            L_L_L2_DCA_PV_ME_SE.push_back(L_DCA_PV_vector.at(1));
          }
        }
      ...
      if( Lbar_vector.size() > 1 && Lbar_Lbar_Lbar1_vector_ME_SE.size() < 3e4)
      {
        ...
        {
          //Minv pre-selection to boost usable ME statistics
          if( Lbar_vector.at(0).M() > 1.105 && Lbar_vector.at(0).M() < 1.13 && Lbar_vector.at(1).M() > 1.105 && Lbar_vector.at(1).M() < 1.13 /*&& Lbar_DCA_PV_vector.at(0) < L_DCA_cut  && Lbar_DCA_PV_vector.at(1) < L_DCA_cut*/)
          {
            ...
            Lbar_Lbar_Lbar1_DCA_PV_ME_SE.push_back(Lbar_DCA_PV_vector.at(0));
            ...
            Lbar_Lbar_Lbar2_DCA_PV_ME_SE.push_back(Lbar_DCA_PV_vector.at(1));
          }
        }
      }
      ...
      if( test == 0 ) nSingle_ME = 1e5;
      else nSingle_ME = 1e4;

      if( L_vector.size() == 1 && Lbar_vector.size() == 0 && L_vector_ME.size() < nSingle_ME)
      {
        //Minv pre-selection to boost usable ME statistics
        if( L_vector.at(0).M() > 1.105 && L_vector.at(0).M() < 1.13 /*&& L_DCA_PV_vector.at(0) < L_DCA_cut*/)
        {
          ...
          L_DCA_PV_vector_ME.push_back(L_DCA_PV_vector.at(0));
        }
      }

      if( L_vector.size() == 0 && Lbar_vector.size() == 1 && Lbar_vector_ME.size() < nSingle_ME /*&& Lbar_DCA_PV_vector.at(0) < L_DCA_cut*/)
      {
        //Minv pre-selection to boost usable ME statistics
        if( Lbar_vector.at(0).M() > 1.105 && Lbar_vector.at(0).M() < 1.13 )
        {
          ...
          Lbar_DCA_PV_vector_ME.push_back(Lbar_DCA_PV_vector.at(0));
        }
      }
      ...
      if( L_vector.size() > 0 && Lbar_vector_background.size() > 0 && L_Lbar_L_vector_ME_SE_back_US_LS.size() < 3e4)
      {
        ...
        if( pi_tag_vector.at(0) != piBar_tag_vector_background.at(0) && pi_tag_vector.at(0) != pBar_tag_vector_background.at(0) )
        {
          ...
          if( L_vector.at(0).M() > 1.105 && L_vector.at(0).M() < 1.13 && Lbar_vector_background.at(0).M() > 1.105 && Lbar_vector_background.at(0).M() < 1.13)
          {
            ...
            L_Lbar_L_DCA_PV_ME_SE_back_US_LS.push_back(L_DCA_PV_vector.at(0));
            ...
            L_Lbar_Lbar_DCA_PV_ME_SE_back_US_LS.push_back(Lbar_DCA_PV_vector_background.at(0));
      ...
      if( L_vector_background.size() > 0 && Lbar_vector.size() > 0 && L_Lbar_L_vector_ME_SE_back_LS_US.size() < 3e4)
      {
        ...
        if( pi_tag_vector_background.at(0) != piBar_tag_vector.at(0) && p_tag_vector_background.at(0) != piBar_tag_vector.at(0) )
        {
          ...
          if( L_vector_background.at(0).M() > 1.105 && L_vector_background.at(0).M() < 1.13 && Lbar_vector.at(0).M() > 1.105 && Lbar_vector.at(0).M() < 1.13)
          {
            ...
            L_Lbar_L_DCA_PV_ME_SE_back_LS_US.push_back(L_DCA_PV_vector_background.at(0));
            ...
            L_Lbar_Lbar_DCA_PV_ME_SE_back_US_LS.push_back(Lbar_DCA_PV_vector.at(0));
          }
        }
      }
      ...
      if( L_vector.size() > 0 && L_vector_background.size() > 0 && L_L_L1_vector_ME_SE_back.size() < 3e4)
      {
        ...
        if( p_tag_vector.at(0) != p_tag_vector_background.at(0) && p_tag_vector.at(0) != pi_tag_vector_background.at(0) )
        {
          ...
          if( L_vector.at(0).M() > 1.105 && L_vector.at(0).M() < 1.13 && L_vector_background.at(0).M() > 1.105 && L_vector_background.at(0).M() < 1.13)
          {
            ...
            L_L_L1_DCA_PV_ME_SE_back.push_back(L_DCA_PV_vector.at(0));
            ...
            L_L_L2_DCA_PV_ME_SE_back.push_back(L_DCA_PV_vector_background.at(0));
          }
        }
      }
      ...
      if(Lbar_vector.size() > 0 && Lbar_vector_background.size() > 0 && Lbar_Lbar_Lbar1_vector_ME_SE_back.size() < 3e4)
      {
        ...
        if( pBar_tag_vector.at(0) != pBar_tag_vector_background.at(0) && pBar_tag_vector.at(0) != piBar_tag_vector_background.at(0) )
        {
          ...
          if( Lbar_vector.at(0).M() > 1.105 && Lbar_vector.at(0).M() < 1.13 && Lbar_vector_background.at(0).M() > 1.105 && Lbar_vector_background.at(0).M() < 1.13)
          {
            ...
            Lbar_Lbar_Lbar1_DCA_PV_ME_SE_back.push_back(Lbar_DCA_PV_vector.at(0));
            ...
            Lbar_Lbar_Lbar2_DCA_PV_ME_SE_back.push_back(Lbar_DCA_PV_vector_background.at(0));
      ...
      if( L_vector_background.size() == 1 && Lbar_vector_background.size() == 0 && L_vector_ME_LS.size() < nSingle_ME)
      {
        if( L_vector_background.at(0).M() > 1.105 && L_vector_background.at(0).M() < 1.13)
        {
          ...
          L_DCA_PV_vector_ME_LS.push_back(L_DCA_PV_vector_background.at(0));

        }
      }

      if( L_vector_background.size() == 0 && Lbar_vector_background.size() == 1 && Lbar_vector_ME_LS.size() < nSingle_ME)
      {
        if( Lbar_vector_background.at(0).M() > 1.105 && Lbar_vector_background.at(0).M() < 1.13)
        {
          ...
          Lbar_DCA_PV_vector_ME_LS.push_back(Lbar_DCA_PV_vector_background.at(0));
      ...
      L_DCA_PV_vector_background.clear();
      ...
      Lbar_DCA_PV_vector_background.clear();
  ...
  for(unsigned int pTbin1 = 0; pTbin1 < nPtBins_corr; pTbin1++)
  {
    for(unsigned int pTbin2 = 0; pTbin2 < nPtBins_corr; pTbin2++)
    {
      ...
      int binLow = L0_inv_mass_vs_L0bar_inv_mass_US[pTbin1][pTbin2]->GetXaxis()->FindBin(L_mass_PDG-2*L_mass_width_estimate);
      int binHigh = L0_inv_mass_vs_L0bar_inv_mass_US[pTbin1][pTbin2]->GetXaxis()->FindBin(L_mass_PDG+2*L_mass_width_estimate);
      ...
      if(fit_Minv == 1) //gives option to override fitting and set Minv range manually
      {
        ...
        if( pTbin1 == 0 && pTbin2 == 0 )
        {
          if( energy == 200 && year == 2012 )
          {
            ...
            else doubleGauss_Lbar_Lbar->SetParameters(100, 1.116, 0.002, 1.116, 0.002); //tight DCA (0.3)
  ...
  if(cut_type == 0)
  {
    LLbarOutFile = new TFile(Form("./output/ProdPlane/%i/ProdPlane_Lambda_ana_cuts_work.root", year), "recreate"); //analysis cuts
  }
  else if(cut_type == 1)
  {
    LLbarOutFile = new TFile(Form("./output/ProdPlane/%i/ProdPlane_Lambda_topo_tight_cuts_work.root", year), "recreate"); //analysis cuts
  }
  else if(cut_type == 2)
  {
    LLbarOutFile = new TFile(Form("./output/ProdPlane/%i/ProdPlane_Lambda_pT_tight_cuts_work.root", year), "recreate"); //analysis cuts
  }
  else if(cut_type == 3)
  {
    LLbarOutFile = new TFile(Form("./output/ProdPlane/%i/ProdPlane_Lambda_DCA_tight_cuts_work.root", year), "recreate"); //analysis cuts
  }
  else
  {
    cout<<"Wrong cut type"<<endl;
  ...
  TH1F *L0_L0bar_C_vs_q = new TH1F("L0_L0bar_C_vs_q", "L0_L0bar_C_vs_q", 10, 0, 1); //orig. 0, 5 range
  TH1F *L0_L0bar_C_vs_q_ME = new TH1F("L0_L0bar_C_vs_q_ME", "L0_L0bar_C_vs_q_ME", 10, 0, 1);

  //-------------------------------------

  TH2F *L0_L0bar_DCA1_vs_DCA2_US_hist = new TH2F("L0_L0bar_DCA1_vs_DCA2_US_hist", "L0_L0bar_DCA1_vs_DCA2_US_hist", 100, 0, 5, 100, 0, 5);
  //TH2F *L0_L0bar_DCA1_vs_DCA2_US_hist = new TH2F("L0_L0bar_DCA1_vs_DCA2_US_hist", "L0_L0bar_DCA1_vs_DCA2_US_hist", 100, 0, 0.1, 100, 0, 5);
  ...
  TH1F *L0_L0bar_delta_R_US_hist = new TH1F("L0_L0bar_delta_R_US_hist", "L0_L0bar_delta_R_US_hist", 100, 0, 5);
  TH1F *L0_L0bar_delta_R_short_US_hist = new TH1F("L0_L0bar_delta_R_short_US_hist", "L0_L0bar_delta_R_short_US_hist", 100, 0, 5);

  TH1F *L0_L0bar_Q_US_hist = new TH1F("L0_L0bar_Q_US_hist", "L0_L0bar_Q_US_hist", 100, 0, 10);

  TH2F* L0_L0bar_delta_R_vs_Q_US_hist = new TH2F("L0_L0bar_delta_R_vs_Q_US_hist", "L0_L0bar_delta_R_vs_Q_US_hist", 100, 0, 5, 100, 0, 10 );
  ...
  TH2F *L0_L0bar_DCA1_vs_DCA2_US_LS_hist = new TH2F("L0_L0bar_DCA1_vs_DCA2_US_LS_hist", "L0_L0bar_DCA1_vs_DCA2_US_LS_hist", 100, 0, 5, 100, 0, 5);
  ...
  TH1F *L0_L0bar_delta_R_US_LS_hist = new TH1F("L0_L0bar_delta_R_US_LS_hist", "L0_L0bar_delta_R_US_LS_hist", 100, 0, 5);
  TH1F *L0_L0bar_delta_R_short_US_LS_hist = new TH1F("L0_L0bar_delta_R_short_US_LS_hist", "L0_L0bar_delta_R_short_US_LS_hist", 100, 0, 5);

  TH1F *L0_L0bar_Q_US_LS_hist = new TH1F("L0_L0bar_Q_US_LS_hist", "L0_L0bar_Q_US_LS_hist", 100, 0, 10);

  TH2F* L0_L0bar_delta_R_vs_Q_US_LS_hist = new TH2F("L0_L0bar_delta_R_vs_Q_US_LS_hist", "L0_L0bar_delta_R_vs_Q_US_LS_hist", 100, 0, 5, 100, 0, 10 );
  ...
  TH1F *L0_L0_delta_R_US_hist = new TH1F("L0_L0_delta_R_US_hist", "L0_L0_delta_R_US_hist", 100, 0, 5);
  TH1F *L0_L0_Q_US_hist = new TH1F("L0_L0_Q_US_hist", "L0_L0_Q_US_hist", 100, 0, 10);

  TH2F* L0_L0_delta_R_vs_Q_US_hist = new TH2F("L0_L0_delta_R_vs_Q_US_hist", "L0_L0_delta_R_vs_Q_US_hist", 100, 0, 5, 100, 0, 10 );
  ...
  TH1F *L0_L0_delta_R_US_LS_hist = new TH1F("L0_L0_delta_R_US_LS_hist", "L0_L0_delta_R_US_LS_hist", 100, 0, 5);
  TH1F *L0_L0_Q_US_LS_hist = new TH1F("L0_L0_Q_US_LS_hist", "L0_L0_Q_US_LS_hist", 100, 0, 10);

  TH2F* L0_L0_delta_R_vs_Q_US_LS_hist = new TH2F("L0_L0_delta_R_vs_Q_US_LS_hist", "L0_L0_delta_R_vs_Q_US_LS_hist", 100, 0, 5, 100, 0, 10 );
  ...
  TH1F *L0bar_L0bar_delta_R_US_hist = new TH1F("L0bar_L0bar_delta_R_US_hist", "L0bar_L0bar_delta_R_US_hist", 100, 0, 5);
  TH1F *L0bar_L0bar_Q_US_hist = new TH1F("L0bar_L0bar_Q_US_hist", "L0bar_L0bar_Q_US_hist", 100, 0, 10);

  TH2F* L0bar_L0bar_delta_R_vs_Q_US_hist = new TH2F("L0bar_L0bar_delta_R_vs_Q_US_hist", "L0bar_L0bar_delta_R_vs_Q_US_hist", 100, 0, 5, 100, 0, 10 );
  ...
  TH1F *L0bar_L0bar_delta_R_US_LS_hist = new TH1F("L0bar_L0bar_delta_R_US_LS_hist", "L0bar_L0bar_delta_R_US_LS_hist", 100, 0, 5);
  TH1F *L0bar_L0bar_Q_US_LS_hist = new TH1F("L0bar_L0bar_Q_US_LS_hist", "L0bar_L0bar_Q_US_LS_hist", 100, 0, 10);

  TH2F* L0bar_L0bar_delta_R_vs_Q_US_LS_hist = new TH2F("L0bar_L0bar_delta_R_vs_Q_US_LS_hist", "L0bar_L0bar_delta_R_vs_Q_US_LS_hist", 100, 0, 5, 100, 0, 10 );
  ...
  TH2F *L0_L0bar_DCA1_vs_DCA2_US_ME_hist = new TH2F("L0_L0bar_DCA1_vs_DCA2_US_ME_hist", "L0_L0bar_DCA1_vs_DCA2_US_ME_hist", 100, 0, 5, 100, 0, 5);
  ...
  TH2F *L0_L0bar_DCA1_vs_DCA2_US_LS_ME_1_hist = new TH2F("L0_L0bar_DCA1_vs_DCA2_US_LS_ME_1_hist", "L0_L0bar_DCA1_vs_DCA2_US_LS_ME_1_hist", 100, 0, 5, 100, 0, 5);
  ...
  TH2F *L0_L0bar_DCA1_vs_DCA2_US_LS_ME_2_hist = new TH2F("L0_L0bar_DCA1_vs_DCA2_US_LS_ME_2_hist", "L0_L0bar_DCA1_vs_DCA2_US_LS_ME_2_hist", 100, 0, 5, 100, 0, 5);
  ...
  //For scan with fixed Delta phi and different Delta y
  TH2F *L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_hist = new TH2F("L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_hist", "L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_hist", 10, -1, 1, 100, 0, 2);
  TH2F *L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_LS_hist = new TH2F("L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_LS_hist", "L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_LS_hist", 10, -1, 1, 100, 0, 2);

  TH2F *L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_ME_hist = new TH2F("L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_ME_hist", "L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_ME_hist", 10, -1, 1, 100, 0, 2);
  TH2F *L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_LS_ME_hist = new TH2F("L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_LS_ME_hist", "L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_LS_ME_hist", 10, -1, 1, 100, 0, 2);

  //---------------

  //Delta y and Delta phi bins vs. DCA_PV
  //short-range
  TH2F *L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_hist = new TH2F("L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_hist", "L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_hist", 10, -1, 1, 15, 0, 15);
  TH2F *L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_hist = new TH2F("L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_hist", "L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_hist", 10, -1, 1, 15, 0, 15);

  TH2F *L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_ME_hist = new TH2F("L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_ME_hist", "L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_ME_hist", 10, -1, 1, 15, 0, 15);
  TH2F *L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_ME_hist = new TH2F("L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_ME_hist", "L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_ME_hist", 10, -1, 1, 15, 0, 15);

  //long-range
  TH2F *L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_hist = new TH2F("L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_hist", "L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_hist", 10, -1, 1, 15, 0, 15);
  TH2F *L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_hist = new TH2F("L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_hist", "L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_hist", 10, -1, 1, 15, 0, 15);

  TH2F *L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_ME_hist = new TH2F("L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_ME_hist", "L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_ME_hist", 10, -1, 1, 15, 0, 15);
  TH2F *L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_ME_hist = new TH2F("L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_ME_hist", "L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_ME_hist", 10, -1, 1, 15, 0, 15);
  ...
  //Delta R for scan
  //Bin 1 is "in cone", bin 2 is out of cone
  TH2F *L0_L0bar_cos_theta_star_vs_delta_R_scan_US_hist = new TH2F("L0_L0bar_cos_theta_star_vs_delta_R_scan_US_hist", "L0_L0bar_cos_theta_star_vs_delta_R_scan_US_hist", 10, -1, 1, 100, 0, 5);
  TH2F *L0_L0bar_cos_theta_star_vs_delta_R_scan_US_LS_hist = new TH2F("L0_L0bar_cos_theta_star_vs_delta_R_scan_US_LS_hist", "L0_L0bar_cos_theta_star_vs_delta_R_scan_US_LS_hist", 10, -1, 1, 100, 0, 5);

  TH2F *L0_L0bar_cos_theta_star_vs_delta_R_scan_US_ME_hist = new TH2F("L0_L0bar_cos_theta_star_vs_delta_R_scan_US_ME_hist", "L0_L0bar_cos_theta_star_vs_delta_R_scan_US_ME_hist", 10, -1, 1, 100, 0, 5);
  TH2F *L0_L0bar_cos_theta_star_vs_delta_R_scan_US_LS_ME_hist = new TH2F("L0_L0bar_cos_theta_star_vs_delta_R_scan_US_LS_ME_hist", "L0_L0bar_cos_theta_star_vs_delta_R_scan_US_LS_ME_hist", 10, -1, 1, 100, 0, 5);
  ...
  //productin plane histograms
  //Delta y and Delta phi bins
  //Bin 1 is "in cone", bin 2 is out of cone
  TH2F *L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_hist = new TH2F("L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_hist", "L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_hist", 10, -1, 1, 2, 0, 2);
  TH2F *L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_LS_hist = new TH2F("L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_LS_hist", "L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_LS_hist", 10, -1, 1, 2, 0, 2);

  TH2F *L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_ME_hist = new TH2F("L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_ME_hist", "L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_ME_hist", 10, -1, 1, 2, 0, 2);
  TH2F *L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_LS_ME_hist = new TH2F("L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_LS_ME_hist", "L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_LS_ME_hist", 10, -1, 1, 2, 0, 2);

  //---------------

  TH2F *L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_hist = new TH2F("L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_hist", "L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_hist", 10, -1, 1, 2, 0, 2);
  TH2F *L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_LS_hist = new TH2F("L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_LS_hist", "L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_LS_hist", 10, -1, 1, 2, 0, 2);

  TH2F *L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_ME_hist = new TH2F("L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_ME_hist", "L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_ME_hist", 10, -1, 1, 2, 0, 2);
  TH2F *L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_LS_ME_hist = new TH2F("L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_LS_ME_hist", "L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_LS_ME_hist", 10, -1, 1, 2, 0, 2);
  ...
  //Delta y and Delta phi bins vs. DCA_PV
  //short-range
  TH2F *L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_hist = new TH2F("L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_hist", "L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_hist", 10, -1, 1, 15, 0, 15);
  TH2F *L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_hist = new TH2F("L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_hist", "L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_hist", 10, -1, 1, 15, 0, 15);

  TH2F *L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_ME_hist = new TH2F("L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_ME_hist", "L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_ME_hist", 10, -1, 1, 15, 0, 15);
  TH2F *L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_ME_hist = new TH2F("L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_ME_hist", "L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_ME_hist", 10, -1, 1, 15, 0, 15);

  //long-range
  TH2F *L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_hist = new TH2F("L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_hist", "L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_hist", 10, -1, 1, 15, 0, 15);
  TH2F *L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_hist = new TH2F("L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_hist", "L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_hist", 10, -1, 1, 15, 0, 15);

  TH2F *L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_ME_hist = new TH2F("L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_ME_hist", "L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_ME_hist", 10, -1, 1, 15, 0, 15);
  TH2F *L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_ME_hist = new TH2F("L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_ME_hist", "L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_ME_hist", 10, -1, 1, 15, 0, 15);
  ...
  TH2F *L0_L0_cos_theta_star_vs_delta_R_scan_US_hist = new TH2F("L0_L0_cos_theta_star_vs_delta_R_scan_US_hist", "L0_L0_cos_theta_star_vs_delta_R_scan_US_hist", 10, -1, 1, 100, 0, 5);
  TH2F *L0_L0_cos_theta_star_vs_delta_R_scan_US_LS_hist = new TH2F("L0_L0_cos_theta_star_vs_delta_R_scan_US_LS_hist", "L0_L0_cos_theta_star_vs_delta_R_scan_US_LS_hist", 10, -1, 1, 100, 0, 5);
  TH2F *L0_L0_cos_theta_star_vs_delta_R_scan_US_ME_hist = new TH2F("L0_L0_cos_theta_star_vs_delta_R_scan_US_ME_hist", "L0_L0_cos_theta_star_vs_delta_R_scan_US_ME_hist", 10, -1, 1, 100, 0, 5);
  TH2F *L0_L0_cos_theta_star_vs_delta_R_scan_US_LS_ME_hist = new TH2F("L0_L0_cos_theta_star_vs_delta_R_scan_US_LS_ME_hist", "L0_L0_cos_theta_star_vs_delta_R_scan_US_LS_ME_hist", 10, -1, 1, 100, 0, 5);
  ...
  //Delta y and Delta phi bins vs. DCA_PV
  //short-range
  TH2F *L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_hist = new TH2F("L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_hist", "L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_hist", 10, -1, 1, 15, 0, 15);
  TH2F *L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_hist = new TH2F("L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_hist", "L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_hist", 10, -1, 1, 15, 0, 15);

  TH2F *L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_ME_hist = new TH2F("L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_ME_hist", "L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_ME_hist", 10, -1, 1, 15, 0, 15);
  TH2F *L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_ME_hist = new TH2F("L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_ME_hist", "L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_ME_hist", 10, -1, 1, 15, 0, 15);

  //long-range
  TH2F *L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_hist = new TH2F("L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_hist", "L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_hist", 10, -1, 1, 15, 0, 15);
  TH2F *L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_hist = new TH2F("L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_hist", "L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_hist", 10, -1, 1, 15, 0, 15);

  TH2F *L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_ME_hist = new TH2F("L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_ME_hist", "L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_ME_hist", 10, -1, 1, 15, 0, 15);
  TH2F *L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_ME_hist = new TH2F("L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_ME_hist", "L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_ME_hist", 10, -1, 1, 15, 0, 15);
  ...
  //Delta R for scan
  //Bin 1 is "in cone", bin 2 is out of cone
  TH2F *L0bar_L0bar_cos_theta_star_vs_delta_R_scan_US_hist = new TH2F("L0bar_L0bar_cos_theta_star_vs_delta_R_scan_US_hist", "L0bar_L0bar_cos_theta_star_vs_delta_R_scan_US_hist", 10, -1, 1, 100, 0, 5);
  TH2F *L0bar_L0bar_cos_theta_star_vs_delta_R_scan_US_LS_hist = new TH2F("L0bar_L0bar_cos_theta_star_vs_delta_R_scan_US_LS_hist", "L0bar_L0bar_cos_theta_star_vs_delta_R_scan_US_LS_hist", 10, -1, 1, 100, 0, 5);
  TH2F *L0bar_L0bar_cos_theta_star_vs_delta_R_scan_US_ME_hist = new TH2F("L0bar_L0bar_cos_theta_star_vs_delta_R_scan_US_ME_hist", "L0bar_L0bar_cos_theta_star_vs_delta_R_scan_US_ME_hist", 10, -1, 1, 100, 0, 5);
  TH2F *L0bar_L0bar_cos_theta_star_vs_delta_R_scan_US_LS_ME_hist = new TH2F("L0bar_L0bar_cos_theta_star_vs_delta_R_scan_US_LS_ME_hist", "L0bar_L0bar_cos_theta_star_vs_delta_R_scan_US_LS_ME_hist", 10, -1, 1, 100, 0, 5);
  ...
  for(unsigned int L_Lbar_index = 0; L_Lbar_index < L_Lbar_cos_theta.size(); L_Lbar_index++)
  {
        ...
        L_Lbar_Minv_Lbar.at(L_Lbar_index) > Lbar_peak_mean-2*Lbar_peak_sigma && L_Lbar_Minv_Lbar.at(L_Lbar_index) < Lbar_peak_mean+2*Lbar_peak_sigma)
    {
      ...
      {
        L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_hist->Fill(L_Lbar_cos_theta.at(L_Lbar_index), delta_eta);
      }

      L0_L0bar_DCA1_vs_DCA2_US_hist->Fill(L_Lbar_L_DCA_PV.at(L_Lbar_index), L_Lbar_Lbar_DCA_PV.at(L_Lbar_index));

      //-------------------------------------

      float delta_R = sqrt( delta_eta*delta_eta + delta_phi*delta_phi );

      if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
      {
        //fill in-cone
        L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_US_hist->Fill(L_Lbar_cos_theta.at(L_Lbar_index), 0.5);

        L0_L0bar_delta_R_short_US_hist->Fill( delta_R );

        L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_hist->Fill(L_Lbar_cos_theta_prodPlane_1.at(L_Lbar_index), 0.5);
        L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_hist->Fill(L_Lbar_cos_theta_prodPlane_2.at(L_Lbar_index), 0.5);

        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_Lbar_L_DCA_PV.at(L_Lbar_index) < (1.5-DCA_cut_index*0.1) && L_Lbar_Lbar_DCA_PV.at(L_Lbar_index) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_hist->Fill(L_Lbar_cos_theta.at(L_Lbar_index), 0.5+DCA_cut_index  );
          }
        }
      ...
      else
      {
        ...
        L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_hist->Fill(L_Lbar_cos_theta_prodPlane_1.at(L_Lbar_index), 1.5);
        L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_hist->Fill(L_Lbar_cos_theta_prodPlane_2.at(L_Lbar_index), 1.5);

        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_Lbar_L_DCA_PV.at(L_Lbar_index) < (1.5-DCA_cut_index*0.1) && L_Lbar_Lbar_DCA_PV.at(L_Lbar_index) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_hist->Fill(L_Lbar_cos_theta.at(L_Lbar_index), 0.5+DCA_cut_index  );
          }
        }
      ...
      //Delta R = 0.93 is about the same as delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut
      if( delta_R < 1.2 )
      {
        //fill in-cone
  ...
  for(unsigned int L_L_index = 0; L_L_index < L_L_cos_theta.size(); L_L_index++)
  {
        ...
        L_L_Minv_L2.at(L_L_index) > L2_peak_mean-2*L2_peak_sigma && L_L_Minv_L2.at(L_L_index) < L2_peak_mean+2*L2_peak_sigma )
    {
      ...
      if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
      {
        ...
        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_L_L1_DCA_PV.at(L_L_index) < (1.5-DCA_cut_index*0.1) && L_L_L2_DCA_PV.at(L_L_index) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_hist->Fill(L_L_cos_theta.at(L_L_index), 0.5+DCA_cut_index  );
          }
        }
      ...
      else
      {
        ...
        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_L_L1_DCA_PV.at(L_L_index) < (1.5-DCA_cut_index*0.1) && L_L_L2_DCA_PV.at(L_L_index) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_hist->Fill(L_L_cos_theta.at(L_L_index), 0.5+DCA_cut_index  );
          }
        }
      }
  ...
  for(unsigned int Lbar_Lbar_index = 0; Lbar_Lbar_index < Lbar_Lbar_cos_theta.size(); Lbar_Lbar_index++)
  {
        ...
        Lbar_Lbar_Minv_Lbar2.at(Lbar_Lbar_index) > Lbar2_peak_mean-2*Lbar2_peak_sigma && Lbar_Lbar_Minv_Lbar2.at(Lbar_Lbar_index) < Lbar2_peak_mean+2*Lbar2_peak_sigma)
    {
      ...
      if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
      {
        ...
        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( Lbar_Lbar_Lbar1_DCA_PV.at(Lbar_Lbar_index) < (1.5-DCA_cut_index*0.1) && Lbar_Lbar_Lbar2_DCA_PV.at(Lbar_Lbar_index) < (1.5-DCA_cut_index*0.1) )
          {
            L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_hist->Fill(Lbar_Lbar_cos_theta.at(Lbar_Lbar_index), 0.5+DCA_cut_index  );
          }
        }
      ...
      else
      {
        ...
        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( Lbar_Lbar_Lbar1_DCA_PV.at(Lbar_Lbar_index) < (1.5-DCA_cut_index*0.1) && Lbar_Lbar_Lbar2_DCA_PV.at(Lbar_Lbar_index) < (1.5-DCA_cut_index*0.1) )
          {
            L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_hist->Fill(Lbar_Lbar_cos_theta.at(Lbar_Lbar_index), 0.5+DCA_cut_index  );
          }
        }
      }
  ...
  for(unsigned int L_Lbar_index = 0; L_Lbar_index < L_Lbar_cos_theta_back.size(); L_Lbar_index++)
  {
        ...
        L_Lbar_Minv_Lbar_back.at(L_Lbar_index) > Lbar_peak_mean-2*Lbar_peak_sigma && L_Lbar_Minv_Lbar_back.at(L_Lbar_index) < Lbar_peak_mean+2*Lbar_peak_sigma )
    {
      ...
      if(delta_phi < Delta_phi_cut)
      {
        L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_LS_hist->Fill(L_Lbar_cos_theta_back.at(L_Lbar_index), delta_eta, LS_weight);
      }

      L0_L0bar_DCA1_vs_DCA2_US_LS_hist->Fill(L_Lbar_L_DCA_PV_back.at(L_Lbar_index), L_Lbar_Lbar_DCA_PV_back.at(L_Lbar_index));
      ...
      if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
      {
        ...
        L0_L0bar_delta_R_short_US_LS_hist->Fill( delta_R );


        L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_LS_hist->Fill(L_Lbar_cos_theta_prodPlane_1_back.at(L_Lbar_index), 0.5, LS_weight);
        L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_LS_hist->Fill(L_Lbar_cos_theta_prodPlane_2_back.at(L_Lbar_index), 0.5, LS_weight);

        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_Lbar_L_DCA_PV_back.at(L_Lbar_index) < (1.5-DCA_cut_index*0.1) && L_Lbar_L_DCA_PV_back.at(L_Lbar_index) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_hist->Fill(L_Lbar_cos_theta_back.at(L_Lbar_index), 0.5+DCA_cut_index, LS_weight  );
          }
        }
      ...
      else
      {
        ...
        L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_LS_hist->Fill(L_Lbar_cos_theta_prodPlane_1_back.at(L_Lbar_index), 1.5, LS_weight);
        L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_LS_hist->Fill(L_Lbar_cos_theta_prodPlane_2_back.at(L_Lbar_index), 1.5, LS_weight);

        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_Lbar_L_DCA_PV_back.at(L_Lbar_index) < (1.5-DCA_cut_index*0.1) && L_Lbar_L_DCA_PV_back.at(L_Lbar_index) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_hist->Fill(L_Lbar_cos_theta_back.at(L_Lbar_index), 0.5+DCA_cut_index, LS_weight  );
          }
        }
  ...
  for(unsigned int L_L_index = 0; L_L_index < L_L_cos_theta_back.size(); L_L_index++)
  {
        ...
        L_L_Minv_L2_back.at(L_L_index) > L2_peak_mean-2*L2_peak_sigma && L_L_Minv_L2_back.at(L_L_index) < L2_peak_mean+2*L2_peak_sigma)
    {
      ...
      if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
      {
        ...
        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_L_L1_DCA_PV_back.at(L_L_index) < (1.5-DCA_cut_index*0.1) && L_L_L2_DCA_PV_back.at(L_L_index) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_hist->Fill(L_L_cos_theta_back.at(L_L_index), 0.5+DCA_cut_index, LS_weight  );
          }
        }
      ...
      else
      {
        ...
        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_L_L1_DCA_PV_back.at(L_L_index) < (1.5-DCA_cut_index*0.1) && L_L_L2_DCA_PV_back.at(L_L_index) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_hist->Fill(L_L_cos_theta_back.at(L_L_index), 0.5+DCA_cut_index, LS_weight  );
          }
        }
  ...
  for(unsigned int Lbar_Lbar_index = 0; Lbar_Lbar_index < Lbar_Lbar_cos_theta_back.size(); Lbar_Lbar_index++)
  {
        ...
        Lbar_Lbar_Minv_Lbar2_back.at(Lbar_Lbar_index) > Lbar2_peak_mean-2*Lbar2_peak_sigma && Lbar_Lbar_Minv_Lbar2_back.at(Lbar_Lbar_index) < Lbar2_peak_mean+2*Lbar2_peak_sigma)
    {
      ...
      if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
      {
        ...
        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( Lbar_Lbar_Lbar1_DCA_PV_back.at(Lbar_Lbar_index) < (1.5-DCA_cut_index*0.1) && Lbar_Lbar_Lbar2_DCA_PV_back.at(Lbar_Lbar_index) < (1.5-DCA_cut_index*0.1) )
          {
            L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_hist->Fill(Lbar_Lbar_cos_theta_back.at(Lbar_Lbar_index), 0.5+DCA_cut_index, LS_weight  );
          }
        }
      ...
      else
      {
        ...
        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( Lbar_Lbar_Lbar1_DCA_PV_back.at(Lbar_Lbar_index) < (1.5-DCA_cut_index*0.1) && Lbar_Lbar_Lbar2_DCA_PV_back.at(Lbar_Lbar_index) < (1.5-DCA_cut_index*0.1) )
          {
            L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_hist->Fill(Lbar_Lbar_cos_theta_back.at(Lbar_Lbar_index), 0.5+DCA_cut_index, LS_weight  );
          }
        }
  ...
  vector<float> n_fill_LLbar_DCA_cut_weight[15]; //array of vectors for weighs with DCA cuts
  ...
  for(unsigned int iLambda_ME = 0; iLambda_ME < L_Lbar_L_vector_ME_SE.size(); iLambda_ME++)
  {
    ...
    int fill_LLbar_DCA_cut_ME[15]; //flag indicating if given SE pair was already used - want to use just once

    for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
    {
      fill_LLbar_DCA_cut_ME[DCA_cut_index] = 0; //first set all weights to 0
    }
    ...
    for(unsigned int iLambdaBar_ME = 0; iLambdaBar_ME < Lbar_vector_ME.size(); iLambdaBar_ME++)
    {
      ...
      //if( fabs( L_Lbar_Lbar_DCA_PV_ME_SE.at(iLambda_ME) - Lbar_DCA_PV_vector_ME.at(iLambdaBar_ME) ) > 0.025 ) continue;
      ...
      for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
      {
        if( L_Lbar_L_DCA_PV_ME_SE.at(iLambda_ME) < (1.5-DCA_cut_index*0.1) && L_Lbar_Lbar_DCA_PV_ME_SE.at(iLambda_ME) < (1.5-DCA_cut_index*0.1) && Lbar_DCA_PV_vector_ME.at(iLambdaBar_ME) < (1.5-DCA_cut_index*0.1) )
        {
          fill_LLbar_DCA_cut_ME[DCA_cut_index]++;
        }
      }
    ...
    for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
    {
      if( fill_LLbar_DCA_cut_ME[DCA_cut_index] > 0) n_fill_LLbar_DCA_cut_weight[DCA_cut_index].push_back(1./fill_LLbar_DCA_cut_ME[DCA_cut_index]);
      else n_fill_LLbar_DCA_cut_weight[DCA_cut_index].push_back(0);
    }
  ...
  for(unsigned int iLambda_ME = 0; iLambda_ME < L_Lbar_L_vector_ME_SE.size(); iLambda_ME++)
  {
    ...
    for(unsigned int iLambdaBar_ME = 0; iLambdaBar_ME < Lbar_vector_ME.size(); iLambdaBar_ME++)
    {
      ...
      pair<double,double> L_Lbar_pairThetaStar_prodPlane = LpairThetaStar_prodPlane(L_Lbar_L_vector_ME_SE.at(iLambda_ME), L_Lbar_p_vector_ME_SE.at(iLambda_ME), Lbar_vector_ME.at(iLambdaBar_ME), pBar_vector_ME.at(iLambdaBar_ME));
      ...
      L0_L0bar_DCA1_vs_DCA2_US_ME_hist->Fill( L_Lbar_L_DCA_PV_ME_SE.at(iLambda_ME), Lbar_DCA_PV_vector_ME.at(iLambdaBar_ME), n_fill_LLbar_weight.at(iLambda_ME)  );

      //-------------------------------------

      if(delta_phi < Delta_phi_cut)
      {
        L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar), delta_eta, n_fill_LLbar_weight.at(iLambda_ME));
      }
      ...
      if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
      {
        ...
        L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.first), 0.5, n_fill_LLbar_weight.at(iLambda_ME));
        //L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.second), 0.5, n_fill_LLbar_weight.at(iLambda_ME));

        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_Lbar_L_DCA_PV_ME_SE.at(iLambda_ME) < (1.5-DCA_cut_index*0.1) && L_Lbar_Lbar_DCA_PV_ME_SE.at(iLambda_ME) < (1.5-DCA_cut_index*0.1) && Lbar_DCA_PV_vector_ME.at(iLambdaBar_ME) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar), 0.5+DCA_cut_index, n_fill_LLbar_DCA_cut_weight[DCA_cut_index].at(iLambda_ME) );
          }
      ...
      else
      {
        ...
        L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.first), 1.5, n_fill_LLbar_weight.at(iLambda_ME));
        //L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.second), 1.5, n_fill_LLbar_weight.at(iLambda_ME));

        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_Lbar_L_DCA_PV_ME_SE.at(iLambda_ME) < (1.5-DCA_cut_index*0.1) && L_Lbar_Lbar_DCA_PV_ME_SE.at(iLambda_ME) < (1.5-DCA_cut_index*0.1) && Lbar_DCA_PV_vector_ME.at(iLambdaBar_ME) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar), 0.5+DCA_cut_index, n_fill_LLbar_DCA_cut_weight[DCA_cut_index].at(iLambda_ME)  );
          }
        }
  ...
  for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
  {
    n_fill_LLbar_DCA_cut_weight[DCA_cut_index].clear();
  }
  ...
  for(unsigned int iLambdaBar_ME = 0; iLambdaBar_ME < L_Lbar_Lbar_vector_ME_SE.size(); iLambdaBar_ME++)
  {
    ...
    for(unsigned int iLambda_ME = 0; iLambda_ME < L_vector_ME.size(); iLambda_ME++)
    {
      ...
      //if( fabs( L_DCA_PV_vector_ME.at(iLambda_ME) -  L_Lbar_L_DCA_PV_ME_SE.at(iLambdaBar_ME) ) > 0.025 ) continue;
      ...
      for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
      {
        ...
        if( L_DCA_PV_vector_ME.at(iLambda_ME) < (1.5-DCA_cut_index*0.1) && L_Lbar_L_DCA_PV_ME_SE.at(iLambdaBar_ME) < (1.5-DCA_cut_index*0.1) && L_Lbar_Lbar_DCA_PV_ME_SE.at(iLambdaBar_ME) < (1.5-DCA_cut_index*0.1) )
        {
          fill_LLbar_DCA_cut_ME[DCA_cut_index]++;
        }
      }
  ...
  for(unsigned int iLambdaBar_ME = 0; iLambdaBar_ME < L_Lbar_Lbar_vector_ME_SE.size(); iLambdaBar_ME++)
  {
    ...
    for(unsigned int iLambda_ME = 0; iLambda_ME < L_vector_ME.size(); iLambda_ME++)
    {
      ...
      pair<double,double> L_Lbar_pairThetaStar_prodPlane = LpairThetaStar_prodPlane(L_vector_ME.at(iLambda_ME), p_vector_ME.at(iLambda_ME), L_Lbar_Lbar_vector_ME_SE.at(iLambdaBar_ME), L_Lbar_pBar_vector_ME_SE.at(iLambdaBar_ME));
      ...
      L0_L0bar_DCA1_vs_DCA2_US_ME_hist->Fill( L_DCA_PV_vector_ME.at(iLambda_ME), L_Lbar_Lbar_DCA_PV_ME_SE.at(iLambdaBar_ME), n_fill_LLbar_weight.at(iLambdaBar_ME)  );

      //---------------------------------------

      if(delta_phi < Delta_phi_cut)
      {
        L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar), delta_eta, n_fill_LLbar_weight.at(iLambdaBar_ME));
      }
      ...
      if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
      {
        ...
        //L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.first), 0.5, n_fill_LLbar_weight.at(iLambdaBar_ME));
        L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.second), 0.5, n_fill_LLbar_weight.at(iLambdaBar_ME));

        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_DCA_PV_vector_ME.at(iLambda_ME) < (1.5-DCA_cut_index*0.1) && L_Lbar_L_DCA_PV_ME_SE.at(iLambdaBar_ME) < (1.5-DCA_cut_index*0.1) && L_Lbar_Lbar_DCA_PV_ME_SE.at(iLambdaBar_ME) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar), 0.5+DCA_cut_index, n_fill_LLbar_DCA_cut_weight[DCA_cut_index].at(iLambdaBar_ME)  );
          }
        }
      ...
      else
      {
        ...
        //L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.first), 1.5, n_fill_LLbar_weight.at(iLambdaBar_ME));
        L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.second), 1.5, n_fill_LLbar_weight.at(iLambdaBar_ME));

        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_DCA_PV_vector_ME.at(iLambda_ME) < (1.5-DCA_cut_index*0.1) && L_Lbar_L_DCA_PV_ME_SE.at(iLambdaBar_ME) < (1.5-DCA_cut_index*0.1) && L_Lbar_Lbar_DCA_PV_ME_SE.at(iLambdaBar_ME) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar), 0.5+DCA_cut_index, n_fill_LLbar_DCA_cut_weight[DCA_cut_index].at(iLambdaBar_ME)  );
          }
        }
      }
    ...
    for(unsigned int iLambda_ME_2 = 0; iLambda_ME_2 < L_vector_ME.size(); iLambda_ME_2++)
    {
      ...
      //{
        ...
        if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
        {
          ...
          for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
          {
            if( L_L_L1_DCA_PV_ME_SE.at(iLambda_ME_1) < (1.5-DCA_cut_index*0.1) && L_DCA_PV_vector_ME.at(iLambda_ME_2) < (1.5-DCA_cut_index*0.1) )
            {
              L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_ME_hist->Fill(TMath::Cos(L_L_pairThetaStar), 0.5+DCA_cut_index, n_fill_LL_weight.at(iLambda_ME_1)  );
            }
          }
        ...
        else
        {
          ...
          for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
          {
            if( L_L_L1_DCA_PV_ME_SE.at(iLambda_ME_1) < (1.5-DCA_cut_index*0.1) && L_DCA_PV_vector_ME.at(iLambda_ME_2) < (1.5-DCA_cut_index*0.1) )
            {
              L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_ME_hist->Fill(TMath::Cos(L_L_pairThetaStar), 0.5+DCA_cut_index, n_fill_LL_weight.at(iLambda_ME_1)  );
            }
          }
        }
    ...
    for(unsigned int iLambdaBar_ME_2 = 0; iLambdaBar_ME_2 < Lbar_vector_ME.size(); iLambdaBar_ME_2++)
    {
      ...
      //{
        ...
        if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
        {
          ...
          for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
          {
            if( Lbar_Lbar_Lbar1_DCA_PV_ME_SE.at(iLambdaBar_ME_1) < (1.5-DCA_cut_index*0.1) && Lbar_DCA_PV_vector_ME.at(iLambdaBar_ME_2) < (1.5-DCA_cut_index*0.1) )
            {
              L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_ME_hist->Fill(TMath::Cos(Lbar_Lbar_pairThetaStar), 0.5+DCA_cut_index, n_fill_LbarLbar_weight.at(iLambdaBar_ME_1)  );
            }
          }
        }
        else
        {
          ...
          for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
          {
            if( Lbar_Lbar_Lbar1_DCA_PV_ME_SE.at(iLambdaBar_ME_1) < (1.5-DCA_cut_index*0.1) && Lbar_DCA_PV_vector_ME.at(iLambdaBar_ME_2) < (1.5-DCA_cut_index*0.1) )
            {
              L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_ME_hist->Fill(TMath::Cos(Lbar_Lbar_pairThetaStar), 0.5+DCA_cut_index, n_fill_LbarLbar_weight.at(iLambdaBar_ME_1)  );
            }
          }
        }
  ...
  for(unsigned int iLambda_ME = 0; iLambda_ME < L_Lbar_L_vector_ME_SE_back_US_LS.size(); iLambda_ME++)
  {
    ...
    for(unsigned int iLambdaBar_ME = 0; iLambdaBar_ME < Lbar_vector_ME_LS.size(); iLambdaBar_ME++)
    {
      ...
      //if( fabs( L_Lbar_Lbar_DCA_PV_ME_SE_back_US_LS.at(iLambda_ME) - Lbar_DCA_PV_vector_ME_LS.at(iLambdaBar_ME) ) > 0.025 ) continue;
  ...
  for(unsigned int iLambda_ME = 0; iLambda_ME < L_Lbar_L_vector_ME_SE_back_US_LS.size(); iLambda_ME++)
  {
    ...
    for(unsigned int iLambdaBar_ME = 0; iLambdaBar_ME < Lbar_vector_ME_LS.size(); iLambdaBar_ME++)
    {
      ...
      pair<double,double> L_Lbar_pairThetaStar_prodPlane = LpairThetaStar_prodPlane(L_Lbar_L_vector_ME_SE_back_US_LS.at(iLambda_ME), L_Lbar_p_vector_ME_SE_back_US_LS.at(iLambda_ME), Lbar_vector_ME_LS.at(iLambdaBar_ME), pBar_vector_ME_LS.at(iLambdaBar_ME));
      ...
      if( fabs(L_Lbar_L_vector_ME_SE_back_US_LS.at(iLambda_ME).Phi() - Lbar_vector_ME_LS.at(iLambdaBar_ME).Phi()) <= TMath::Pi() )
      {
        delta_phi = fabs(L_Lbar_L_vector_ME_SE_back_US_LS.at(iLambda_ME).Phi() - Lbar_vector_ME_LS.at(iLambdaBar_ME).Phi());
      }
      else
      {
        delta_phi = 2*TMath::Pi() - fabs(L_Lbar_L_vector_ME_SE_back_US_LS.at(iLambda_ME).Phi() - Lbar_vector_ME_LS.at(iLambdaBar_ME).Phi()) ;
      }
      ...
      L0_L0bar_DCA1_vs_DCA2_US_LS_ME_1_hist->Fill( L_Lbar_L_DCA_PV_ME_SE_back_US_LS.at(iLambda_ME), Lbar_DCA_PV_vector_ME_LS.at(iLambdaBar_ME), n_fill_LLbar_back_weight.at(iLambda_ME)  );

      //-------------------------------------

      if(delta_phi < Delta_phi_cut)
      {
        L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar), delta_eta, n_fill_LLbar_back_weight.at(iLambda_ME));
      }
      ...
      if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
      {
        ...
        L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.first), 0.5, n_fill_LLbar_back_weight.at(iLambda_ME));
        L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.second), 0.5, n_fill_LLbar_back_weight.at(iLambda_ME));

        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_Lbar_L_DCA_PV_ME_SE_back_US_LS.at(iLambda_ME) < (1.5-DCA_cut_index*0.1) && Lbar_DCA_PV_vector_ME_LS.at(iLambdaBar_ME) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar), 0.5+DCA_cut_index, n_fill_LLbar_back_weight.at(iLambda_ME)  );
          }
        }
      ...
      else
      {
        ...
        L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.first), 1.5, n_fill_LLbar_back_weight.at(iLambda_ME));
        L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.second), 1.5, n_fill_LLbar_back_weight.at(iLambda_ME));

        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_Lbar_L_DCA_PV_ME_SE_back_US_LS.at(iLambda_ME) < (1.5-DCA_cut_index*0.1) && Lbar_DCA_PV_vector_ME_LS.at(iLambdaBar_ME) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar), 0.5+DCA_cut_index, n_fill_LLbar_back_weight.at(iLambda_ME)  );
          }
        }
      }
  ...
  for(unsigned int iLambdaBar_ME = 0; iLambdaBar_ME < L_Lbar_Lbar_vector_ME_SE_back_LS_US.size(); iLambdaBar_ME++)
  {
    ...
    for(unsigned int iLambda_ME = 0; iLambda_ME < L_vector_ME_LS.size(); iLambda_ME++)
    {
      ...
      //if( fabs( L_DCA_PV_vector_ME_LS.at(iLambda_ME) - L_Lbar_L_DCA_PV_ME_SE_back_LS_US.at(iLambdaBar_ME) ) > 0.025 ) continue;
  ...
  for(unsigned int iLambdaBar_ME = 0; iLambdaBar_ME < L_Lbar_Lbar_vector_ME_SE_back_LS_US.size(); iLambdaBar_ME++)
  {
    ...
    for(unsigned int iLambda_ME = 0; iLambda_ME < L_vector_ME_LS.size(); iLambda_ME++)
    {
      ...
      pair<double,double> L_Lbar_pairThetaStar_prodPlane = LpairThetaStar_prodPlane(L_vector_ME_LS.at(iLambda_ME), p_vector_ME_LS.at(iLambda_ME), L_Lbar_Lbar_vector_ME_SE_back_LS_US.at(iLambdaBar_ME), L_Lbar_pBar_vector_ME_SE_back_LS_US.at(iLambdaBar_ME));
      ...
      if( fabs(L_vector_ME_LS.at(iLambda_ME).Phi() - L_Lbar_Lbar_vector_ME_SE_back_LS_US.at(iLambdaBar_ME).Phi()) <= TMath::Pi() )
      {
        delta_phi = fabs(L_vector_ME_LS.at(iLambda_ME).Phi() - L_Lbar_Lbar_vector_ME_SE_back_LS_US.at(iLambdaBar_ME).Phi());
      }
      else
      {
        delta_phi = 2*TMath::Pi() - fabs(L_vector_ME_LS.at(iLambda_ME).Phi() - L_Lbar_Lbar_vector_ME_SE_back_LS_US.at(iLambdaBar_ME).Phi()) ;
      }
      ...
      L0_L0bar_DCA1_vs_DCA2_US_LS_ME_2_hist->Fill( L_DCA_PV_vector_ME_LS.at(iLambda_ME), L_Lbar_Lbar_DCA_PV_ME_SE_back_US_LS.at(iLambdaBar_ME), n_fill_LLbar_back_weight.at(iLambdaBar_ME)  );

      //-------------------------------------

      if(delta_phi < Delta_phi_cut)
      {
        L0_L0bar_cos_theta_star_vs_delta_eta_delta_phi_2_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar), delta_eta, n_fill_LLbar_back_weight.at(iLambdaBar_ME));
      }
      ...
      if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
      {
        ...
        L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.first), 0.5, n_fill_LLbar_back_weight.at(iLambdaBar_ME));
        L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.second), 0.5, n_fill_LLbar_back_weight.at(iLambdaBar_ME));

        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_DCA_PV_vector_ME_LS.at(iLambda_ME) < (1.5-DCA_cut_index*0.1) && L_Lbar_Lbar_DCA_PV_ME_SE_back_US_LS.at(iLambdaBar_ME) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar), 0.5+DCA_cut_index, n_fill_LLbar_back_weight.at(iLambdaBar_ME)  );
          }
        }
      }
      else
      {
        ...
        L0_L0bar_cos_theta_star_prodPlane1_vs_delta_eta_delta_phi_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.first), 1.5, n_fill_LLbar_back_weight.at(iLambdaBar_ME));
        L0_L0bar_cos_theta_star_prodPlane2_vs_delta_eta_delta_phi_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar_prodPlane.second), 1.5, n_fill_LLbar_back_weight.at(iLambdaBar_ME));

        for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
        {
          if( L_DCA_PV_vector_ME_LS.at(iLambda_ME) < (1.5-DCA_cut_index*0.1) && L_Lbar_Lbar_DCA_PV_ME_SE_back_US_LS.at(iLambdaBar_ME) < (1.5-DCA_cut_index*0.1) )
          {
            L0_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_ME_hist->Fill(TMath::Cos(L_Lbar_pairThetaStar), 0.5+DCA_cut_index, n_fill_LLbar_back_weight.at(iLambdaBar_ME)  );
          }
        }
      }
    ...
    for(unsigned int iLambda_ME_2 = 0; iLambda_ME_2 < L_vector_ME_LS.size(); iLambda_ME_2++)
    {
      ...
      //{
        ...
        if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
        {
          ...
          for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
          {
            if( L_L_L1_DCA_PV_ME_SE_back.at(iLambda_ME_1) < (1.5-DCA_cut_index*0.1) && L_DCA_PV_vector_ME_LS.at(iLambda_ME_2) < (1.5-DCA_cut_index*0.1) )
            {
              L0_L0_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_ME_hist->Fill(TMath::Cos(L_L_pairThetaStar), 0.5+DCA_cut_index, n_fill_LL_back_weight.at(iLambda_ME_1)  );
            }
          }
        ...
        else
        {
          ...
          for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
          {
            if( L_L_L1_DCA_PV_ME_SE_back.at(iLambda_ME_1) < (1.5-DCA_cut_index*0.1) && L_DCA_PV_vector_ME_LS.at(iLambda_ME_2) < (1.5-DCA_cut_index*0.1) )
            {
              L0_L0_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_ME_hist->Fill(TMath::Cos(L_L_pairThetaStar), 0.5+DCA_cut_index, n_fill_LL_back_weight.at(iLambda_ME_1)  );
            }
          }
        }
    ...
    for(unsigned int iLambdaBar_ME_2 = 0; iLambdaBar_ME_2 < Lbar_vector_ME_LS.size(); iLambdaBar_ME_2++)
    {
      ...
      //{
        ...
        if( delta_eta < Delta_y_cut && delta_phi < Delta_phi_cut )
        {
          ...
          for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
          {
            if( Lbar_Lbar_Lbar1_DCA_PV_ME_SE_back.at(iLambdaBar_ME_1) < (1.5-DCA_cut_index*0.1) && Lbar_DCA_PV_vector_ME_LS.at(iLambdaBar_ME_2) < (1.5-DCA_cut_index*0.1) )
            {
              L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_1_vs_DCA_PV_US_LS_ME_hist->Fill(TMath::Cos(Lbar_Lbar_pairThetaStar), 0.5+DCA_cut_index, n_fill_LbarLbar_back_weight.at(iLambdaBar_ME_1)  );
            }
          }
        ...
        else
        {
          ...
          for(unsigned int DCA_cut_index = 0; DCA_cut_index < 15; DCA_cut_index++)
          {
            if( Lbar_Lbar_Lbar1_DCA_PV_ME_SE_back.at(iLambdaBar_ME_1) < (1.5-DCA_cut_index*0.1) && Lbar_DCA_PV_vector_ME_LS.at(iLambdaBar_ME_2) < (1.5-DCA_cut_index*0.1) )
            {
              L0bar_L0bar_cos_theta_star_delta_eta_delta_phi_2_vs_DCA_PV_US_LS_ME_hist->Fill(TMath::Cos(Lbar_Lbar_pairThetaStar), 0.5+DCA_cut_index, n_fill_LbarLbar_back_weight.at(iLambdaBar_ME_1)  );
            }
          }
...
void Lambda_corr_2D(const int ReadMode = 0, const int cut_type = 0, const int energy = 510, const int year = 2017, const int test = 0)
{
  ifstream fileList;
  ...
  if(energy == 200)
  {
    if(year == 2012)
    {
      fileList.open("./input/data/fileList.list");
    }
  ...
  bool AnaFinish = DoAnalysis(myChain, ReadMode, cut_type, energy , year, test);
