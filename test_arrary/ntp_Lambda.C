...
{  
   ...
   TH1D *h1D_NLambda        = new TH1D("h1D_NLambda","h1D_NLambda",17,-1.5,15.5);
   ...
   for(int iFile = 0 ; iFile < InPutFileList.size(); iFile++){
      ...
      for(int iEntry = 0 ; iEntry < Nentries ; iEntry++){
         ...
         for(int iLam = 0 ; iLam < NLambda; iLam++){
            h1D_p1_pt                ->Fill(p1_pt[iLam]  );  
            h1D_p1_phi               ->Fill(p1_phi[iLam] );  
            h1D_p1_eta               ->Fill(p1_eta[iLam] );  
            h1D_p1_dca               ->Fill(p1_dca[iLam] );  
            h1D_p1_ch                ->Fill(p1_ch[iLam] );  
            h1D_p1_hasTOFinfo        ->Fill(p1_hasTOFinfo[iLam] );  
            h1D_p2_pt                ->Fill(p2_pt[iLam]);  
            h1D_p2_phi               ->Fill(p2_phi[iLam]  );  
            h1D_p2_eta               ->Fill(p2_eta[iLam] );  
            h1D_p2_dca               ->Fill(p2_dca[iLam] );  
            h1D_p2_hasTOFinfo        ->Fill(p2_hasTOFinfo[iLam]);  
            h1D_pair_charge          ->Fill(pair_charge[iLam] );  
            h1D_pair_DCAdaughters    ->Fill(pair_DCAdaughters[iLam]);  
            h1D_pair_theta           ->Fill(pair_theta[iLam] );  
            h1D_pair_decayL          ->Fill(pair_decayL[iLam]);  
            h1D_pair_phi             ->Fill(pair_phi[iLam]);  
            h1D_pair_eta             ->Fill(pair_eta[iLam]);  
            h1D_pair_pt              ->Fill(pair_pt[iLam]);  
            h1D_pair_mass            ->Fill(pair_mass[iLam] ); 
         }
